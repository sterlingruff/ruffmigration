          ...
          <Card style={{ padding: '24px' }}>
            ...
            <ResponsiveContainer width="100%" height={400}>
              <BarChart
                ...
                layout="vertical"
                margin={{ top: 0, right: 8, bottom: 0, left: 28 }}
                barCategoryGap="6%"
              >
                <XAxis
                  type="number"
                  domain={[-(maxPyramidVal + 0.5), maxPyramidVal + 0.5]}
                  tickFormatter={v => Math.abs(v).toFixed(1) + '%'}
                  tick={{ fontSize: 8, fill: '#aaa', fontFamily: 'Georgia, serif' }}
                />
                <YAxis
                  dataKey="age"
                  type="category"
                  tick={{ fontSize: 9, fill: '#888', fontFamily: 'Georgia, serif' }}
                  width={32}
                />
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
                <Tooltip
                  formatter={(v, name) => [Math.abs(v).toFixed(2) + '%', name === 'male' ? l.male : l.female]}
                  contentStyle={{ fontFamily: 'Georgia, serif', fontSize: '0.8rem', border: `1px solid ${COLORS.border}` }}
                />
                <ReferenceLine x={0} stroke="#ddd" />
                <Bar dataKey="male" fill={COLORS.navy} name="male" radius={[0, 0, 0, 0]} />
                <Bar dataKey="female" fill={COLORS.terracotta} name="female" radius={[0, 0, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </Card>
        </div>
